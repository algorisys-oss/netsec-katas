...
| Customer-managed key (CMEK) | Customer KMS / HSM for disk volumes | Cloud KMS key ring + key; configured per service (GCS, BigQuery, CloudSQL) | AWS KMS customer managed key (AWS deprecated "CMK = Customer Master Key" in favor of "KMS key"; not abbreviated CMK); configured per service (S3, RDS, EBS) | Azure Key Vault (key vault) with customer-managed key; per service opt-in |
