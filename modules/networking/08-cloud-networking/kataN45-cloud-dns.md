...
| VPC-internal resolver (auto) | Configured via DHCP | `169.254.169.254` (link-local metadata server / `metadata.google.internal`); called "Google-provided DNS" | `VPC-base + 2` (e.g. `10.104.0.2`); called "Amazon-provided DNS" | `168.63.129.16` — Azure's multi-purpose platform "wire server" (DNS + health probe + DHCP), not solely a DNS resolver, so the GCP/AWS equivalence is not exactly 1:1 |
