...
| Flow logging | NetFlow/sFlow from switches | VPC Flow Logs per subnet (enable explicitly) | VPC Flow Logs per ENI/subnet (enable explicitly) | NSG Flow Logs (enable explicitly; stored in Storage Account; retiring 30 Jun 2025 → migrate to VNet flow logs) |
| Connectivity blind spot | Unmonitored switch ports; dark VLAN | Intra-VPC east-west if flow logs off; IAP tunnels not in VPC flow | Same-SG same-host traffic not in VPC flow | Intra-VNet traffic without VNet flow logs (NSG flow logs retired 2025) |
